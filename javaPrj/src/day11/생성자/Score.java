...
//학생 성적정보를 담을 수 있는 자료형 틀 만들기
//멤버면수 + 멤버 매서드(멤버변수를 사용하는 매서드)
//멤버매서드는 객체가 생성된 이후부터 사용가능하다.

//type정의함// 주의사항: 변수가 만들어진것이 아니다.
public class Score {
...
//	생성자를 하나도 만들지 않으면 기본생성자를 제공한다.
//	단, 생성자를 하나라도 만들면 기본생성자는 제공되지 않는다.
//	Score Type이 메모리에 생성되었다.
	
//	생성자 오버로딩 합니다.  매개변수가 반드시 달라야한다.
//	오버로딩이란 매서드의 이름은 동일하고 매개변수가 다른경우를 말한다.(개수,type이 다른것을 말함)
//	오버로딩 시 반환타입이 다른것은 오버로딩이 되지 않는다.
	public Score() {} //기본생성자
	
//	생성자에게 객체변수값의 초기화를 담당
